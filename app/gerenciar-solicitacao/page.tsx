...
export default function ManageRequestsPage() {
  ...
  const patchStatus = async (id: string, action: "approve" | "reject") => {
    try {
      ...
      setRequests((prev) =>
        prev.map((r) => (r.id === id ? { ...r, status: newLocalStatus, decidedAt: newDecidedAt } : r))
      )

      setSelectedRequest((prev) =>
        prev && prev.id === id ? { ...prev, status: newLocalStatus, decidedAt: newDecidedAt } : prev
      )
    } catch (e) {
      console.error(e)
  ...
  const handleApprove = (id: string) => patchStatus(id, "approve")
  const handleReject = (id: string) => patchStatus(id, "reject")
  ...
  const getDecisionLabel = (r: Request) => {
    if (r.status === "approved") return `Aprovado em: ${formatDateBR(r.decidedAt)}`
    if (r.status === "rejected") return `Rejeitado em: ${formatDateBR(r.decidedAt)}`
    return null
  }
                        ...
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                          <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                              ...
                              {decisionLabel && <span>{decisionLabel}</span>}
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold">{formatDateBR(selectedRequest.createdAt)}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold">
                      {selectedRequest.status === "pending" ? "—" : formatDateBR(selectedRequest.decidedAt)}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    {selectedRequest.physicals.map((physical, index) => (
                      <div key={index} className="border border-border rounded-lg p-4 bg-white">
                        ...
                        <p className="text-sm text-muted-foreground">
                          <strong>Justificativa:</strong> {physical.justification}
                        </p>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <h4 className="font-semibold text-foreground mb-3">
                    Sazonalização ({selectedRequest.seasonalization.length} meses)
                  </h4>
